...
namespace StrengthSync.Application.Features.Users.Handlers
{
    ...
    {
        public class UserLoginHandler(IUserRepository repository, ILogger<UserLoginHandler> logger) : IRequestHandler<UserLoginCommand, Result<Exception, string>>
        {
            public async Task<Result<Exception, string>> Handle(UserLoginCommand request, CancellationToken cancellationToken)
            {
                logger.LogInformation($"Buscando usuario com o username {request.Username} para realizar o login");

                var user = await repository.GetUserLoginAsync(request.Username);

                if (user == null)
                {
                    logger.LogInformation("Usuario não cadastrado");
                ...
                if (!correctPassword)
                {
                    logger.LogError("Usuario Encontrado, porém com a senha incorreta");
                ...
                if (!permissions.Any())
                {
                    logger.LogError("Usuario sem permissão nenhuma cadastrada");

                    return new BusinessException(ErrorCodes.InvalidObject, "Nenhuma permissão cadastrada. Por favor entre em contato com o suporte");
                }

                logger.LogInformation("Gerando Token de acesso");

                var tokenResult = await GenerateToken(user.Username, permissions);

                if (tokenResult.IsFailure)
                {
                    logger.LogError($"Falha ao gerar o token. Error: {tokenResult.Failure.Message}");
